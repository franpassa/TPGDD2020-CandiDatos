...
AS
BEGIN
INSERT INTO Stock_Sucursal(c_autoparte,n_cantidad_autopartes)
SELECT M.AUTO_PARTE_CODIGO,
(SELECT SUM(COMPRA_CANT) FROM GD2C2020.gd_esquema.Maestra
WHERE COMPRA_NRO IS NOT NULL AND AUTO_PARTE_CODIGO = M.AUTO_PARTE_CODIGO AND AUTO_PARTE_CODIGO IS NOT NULL
AND COMPRA_CANT IS NOT NULL
GROUP BY AUTO_PARTE_CODIGO
HAVING SUM(COMPRA_CANT) IS NOT NULL)
-
(SELECT SUM(CANT_FACTURADA) FROM GD2C2020.gd_esquema.Maestra
WHERE COMPRA_NRO IS NULL AND AUTO_PARTE_CODIGO = M.AUTO_PARTE_CODIGO
GROUP BY AUTO_PARTE_CODIGO)
...
--ORDER BY c_autoparte DESC

--SELECT AUTO_PARTE_CODIGO,SUM(COMPRA_CANT) FROM GD2C2020.gd_esquema.Maestra
--WHERE COMPRA_NRO IS NOT NULL
--GROUP BY AUTO_PARTE_CODIGO
--ORDER BY AUTO_PARTE_CODIGO DESC

--SELECT AUTO_PARTE_CODIGO,SUM(CANT_FACTURADA) FROM GD2C2020.gd_esquema.Maestra
--WHERE COMPRA_NRO IS NULL
--GROUP BY AUTO_PARTE_CODIGO
--ORDER BY AUTO_PARTE_CODIGO DESC
